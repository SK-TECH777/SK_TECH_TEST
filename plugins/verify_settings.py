...
from config import OWNER_ID, VERIFY_MODE, get_verify_mode_value, set_verify_mode_value
...
async def verify_settings_cmd(client, message: Message):
    ...
    await message.reply_text(f"🔐 VERIFY MODE is currently: **{'ON' if mode else 'OFF'}**", reply_markup=kb, parse_mode="html")
...
async def toggle_verify_cb(client, query: CallbackQuery):
    ...
    kb = InlineKeyboardMarkup([[InlineKeyboardButton("Turn OFF" if new else "Turn ON", callback_data="toggle_verify")]])
    await query.message.edit_text(f"✅ VERIFY MODE updated to: **{'ON' if new else 'OFF'}**", reply_markup=kb, parse_mode="html")
